...
public class DateTimeControl extends TextControl {
	/**
	 * 
	 */
	...
	public DateTimeControl(InputTag inputtag) {
		super(inputtag);
	...
	public void setItemField(ItemField item) {
		...
		if (item.isShow()) {
			if (item.isWrite()) {
				...
				if (mThis.getAttribute("size") != null) {
					gsize = Integer.parseInt(mThis.getAttribute("size"));
				}
				if (gsize < 21) {
					mThis.setAttribute("size", "21");
				}
		...
		mThis.setAttribute("type", "datetime");
		mThis.setAttribute("clearable", "true");
		mThis.setAttribute("size", "small");
		mThis.setAttribute("value-format", "yyyy-MM-dd HH:mm:ss");
		mThis.removeAttribute("name");
		mThis.removeAttribute("id");
			...
			if (saTmp != null && saTmp.length > 0) {
				for (int i = 0; i < saTmp.length; i++) {
					if (saTmp[i].equalsIgnoreCase("equstr(this)")) {
						sTmp1.append("equstr($(\"input[name='" + mName + "']\").val());");
					} else {
						sTmp1.append("$(\"input[name='").append(saTmp[i]).append("']\").val()=");
					}
				}
				sTmp = sTmp1.toString();
