...
public class FloatControl_phone extends TextControl_phone {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public FloatControl_phone(InputTag inputtag) {
		super(inputtag);
	}
	...
	public void setItemField(ItemField item) {
		...
		if (item.isShow()) {
			if (item.isWrite()) {
				...
				mThis.setAttribute("onKeyPress", "floatInput()");
				// mThis.setAttribute("class", "text");
				// mThis.setAttribute("class", "easyui-numberbox");
				this.mWrite = true;
			} else {
				if (item.getShowValue() != null && !item.getShowValue().equals("")) {
					mThis.setAttribute("value", item.getShowValue());
	...
	public String getValidateJS() {
		...
		if (this.mWrite) {
			if (this.mItem.isNull() == false) {
				sb.append("if (!checkfloatNull($(\"input[name='").append(mName).append("']\").val(),'")
						.append(mItem.getChineseName()).append("')){\n");
				sb.append("   return false;\n");
				sb.append("}\n");
			} else {
				sb.append("if (!checkfloat($(\"input[name='").append(mName).append("']\").val(),'")
						.append(mItem.getChineseName()).append("')){\n");
				sb.append("   return false;\n");
				sb.append("}\n");
			}
