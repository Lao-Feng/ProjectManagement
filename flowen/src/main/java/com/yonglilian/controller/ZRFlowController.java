...
@Controller
@RestController
@RequestMapping("/zrflow")
public class ZRFlowController extends BaseController {
	/**
	 * 
	 */
...
//	    response.getWriter().flush();
	...
	public String openFlowConfig(ModelMap modelMap, 
	      ...
	      //打开增加按钮-----------------
	          ...
	          String tmp = "";
	          if (Button.getTYPE().equals("1")) {
	             tmp = tmp + "<option value='1' selected>功能按钮</option>\r\n";
	          }else {
	            tmp = tmp + "<option value='1'>功能按钮</option>\r\n";
	          }
	          if (Button.getTYPE().equals("2")) {
	             tmp = tmp + "<option value='2' selected>文档按钮</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='2'>文档按钮</option>\r\n";
	          }
	          if (Button.getTYPE().equals("3")) {
	             tmp = tmp + "<option value='3' selected>控制按钮</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='3'>控制按钮</option>\r\n";
	          }
	          if (Button.getTYPE().equals("4")) {
	             tmp = tmp + "<option value='4' selected>公共功能按钮</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='4'>公共功能按钮</option>\r\n";
	          }
	          if (Button.getTYPE().equals("5")) {
	             tmp = tmp + "<option value='5' selected>公共已分配按钮</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='5'>公共已分配按钮</option>\r\n";
	          }
	          if (Button.getTYPE().equals("6")) {
	             tmp = tmp + "<option value='6' selected>流程创建人按钮</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='6'>流程创建人按钮</option>\r\n";
	          }
	          Button.setTYPE(tmp);
	          tmp = "";
	          if (Button.getPOSITION().equals("1")) {
	             tmp = tmp + "<option value='1' selected>页面上</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='1'>页面上</option>\r\n";
	          }
	          if (Button.getPOSITION().equals("2")) {
	             tmp = tmp + "<option value='2' selected>页面下</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='2'>页面下</option>\r\n";
	          }
	          if (Button.getPOSITION().equals("3")) {
	             tmp = tmp + "<option value='3' selected>页面上和下</option>\r\n";
	          }else {
	             tmp = tmp + "<option value='3'>页面上和下</option>\r\n";
	          }
	          Button.setPOSITION(tmp);
	          modelMap.put("ID",ID);
	          modelMap.put("UNITID",user.getUnitID());
	      ...
	      //打开流程步骤间的关系设计图---------------
	          ...
	          if (Activity!=null && Activity.length>0){
	           ...
	           for(int i=0;i<Activity.length;i++){
	             ...
	             if (i==0) {
	                PData = PData + "{\"id\":\""+SE+Activity[i].getID()+"\",\"flow_id\":\"4\",\"process_name\":\""+Activity[i].getNAME()+"\",\"process_to\":\""+toflow1+"\",\"icon\":\""+flow_ico+"\",\"style\":\"width:"+strWidth+";height:40px;line-height:40px;color:#0e76a8;left:"+XYCSS[5]+";top:"+XYCSS[6]+";\"}";
	             }else{
	                PData = PData + ",{\"id\":\""+SE+Activity[i].getID()+"\",\"flow_id\":\"4\",\"process_name\":\""+Activity[i].getNAME()+"\",\"process_to\":\""+toflow1+"\",\"icon\":\""+flow_ico+"\",\"style\":\"width:"+strWidth+";height:40px;line-height:40px;color:#0e76a8;left:"+XYCSS[5]+";top:"+XYCSS[6]+";\"}";
	             }
	             if (i!=0 && (i+1) % 5==0) {
	      ...
	      if (strAct.equals("FlowShowMap")) {
			...
			if (Activity!=null && Activity.length>0){
			     ...
			     for(int i=0;i<Activity.length;i++){
			       ...
			       if (i==0) {
			         PData = PData + "{\"id\":\""+SE+Activity[i].getID()+"\",\"flow_id\":\"4\",\"process_name\":\""+dq+Activity[i].getNAME()+"\",\"process_to\":\""+toflow1+"\",\"icon\":\""+flow_ico+"\",\"style\":\"width:"+strWidth+";height:40px;line-height:40px;color:#0e76a8;left:"+XYCSS[5]+";top:"+XYCSS[6]+";\"}";
			       }else {
			         PData = PData + ",{\"id\":\""+SE+Activity[i].getID()+"\",\"flow_id\":\"4\",\"process_name\":\""+dq+Activity[i].getNAME()+"\",\"process_to\":\""+toflow1+"\",\"icon\":\""+flow_ico+"\",\"style\":\"width:"+strWidth+";height:40px;line-height:40px;color:#0e76a8;left:"+XYCSS[5]+";top:"+XYCSS[6]+";\"}";
			       }
	...
	public String getnewflowlist(HttpServletRequest request,HttpServletResponse response) throws Exception{
       ...
       if (tempArray != null && tempArray.length > 0) {
           ...
           for (int i = 0; i < tempArray.length; i++) {
                   ...
                   strshow_data = strshow_data + "\"NAME2\":\"<input id='NAME20' name='NAME20' onclick=_OnSet_Tab_NAME20('"+i+"'); type=checkbox value="+tempArraySub[4]+" "+msp.get(tempArraySub[4])+"/>\",";
                   strshow_data = strshow_data + "\"NAME3\":\"<input id='NAME30' name='NAME30' onclick=_OnSet_Tab_NAME30('"+i+"'); type=checkbox value="+tempArraySub[5]+" "+msp.get(tempArraySub[5])+"/>\",";
                   strshow_data = strshow_data + "\"NAME4\":\"<input id='NAME40' name='NAME40' onclick=_OnSet_Tab_NAME40('"+i+"'); type=checkbox value="+tempArraySub[6]+" "+msp.get(tempArraySub[6])+"/>\",";
                   strshow_data = strshow_data + "\"NAME5\":\"<input id='NAME50' name='NAME50' onclick=_OnSet_Tab_NAME50('"+i+"'); type=checkbox value="+tempArraySub[7]+" "+msp.get(tempArraySub[7])+"/>\",";
                   try {
