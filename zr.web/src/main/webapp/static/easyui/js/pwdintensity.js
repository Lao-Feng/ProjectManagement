    ...
    for (var index = 0; index < strWord.length; index++)
    {
        for (var nCharSet = 0; nCharSet < characterSetChecks.length; nCharSet++)
        {
            if (!characterSetChecks[nCharSet].fResult
                    && IsSomeCharType(strWord.charAt(index), characterSetChecks[nCharSet].type))
            {
...
function IsFoundInDictionary(strWord, dictionary)
{
    ...
    return dictionary.Lookup(FormatWord(strWord));
...
function printIntensity(rank) {
	...
	if (rank == 1) {
		//s = "-15";
		verifyPsd.easy($("#PasswordCheck"));
	} else if (rank == 2) {
		verifyPsd.middle($("#PasswordCheck"));
	} else if (rank == 3) {
		verifyPsd.hard($("#PasswordCheck"));
	}else{
		verifyPsd.easy($("#PasswordCheck"));
		}
...
var verifyPsd = {
		easy:function(obj){
			obj.text("不安全").removeAttr("class").addClass("verifyPsd_easy").animate({width:"60px"})},
		middle:function(obj){
			obj.text("普通").removeAttr("class").addClass("verifyPsd_middle").animate({width:"130px"})},
		hard:function(obj){
			obj.text("安全").removeAttr("class").addClass("verifyPsd_hard").animate({width:"156px"})}
	}