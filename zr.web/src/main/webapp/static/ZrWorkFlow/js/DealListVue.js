...
function ListAllVue(form){
	tmpstr="";
	$("select[name='destList'] option").each(function(){
        //遍历所有option
...
function ListAll1Vue(form){
	tmpstr="";
	$("select[name='destList1'] option").each(function(){
        //遍历所有option
        var value = $(this).val();   //获取option值
        var text = $(this).text();
        tmpstr+=value;
...
// vue
function deleteFromDestListPuVue(destList) { 
	console.info(destList)
	var len = destList.length;
	console.info(len)
	for(var i = (len-1); i >= 0; i--) {
...
//将选择好项的数值填充到指定的暂存域中
function fillSelectValueToFieldVue(destList,thisVue,filedname){
	tmpstr="";
	$("select[name="+destList+"] option").each(function(){
        //遍历所有option
        var value = $(this).val();   //获取option值
