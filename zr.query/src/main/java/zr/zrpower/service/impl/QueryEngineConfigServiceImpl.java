...
import zr.zrpower.dao.mapper.QueryConfigMapper;
import zr.zrpower.queryengine.mode.*;
import zr.zrpower.service.QueryEngineConfigService;
 ...
 */
@Service
public class QueryEngineConfigServiceImpl implements QueryEngineConfigService {
	/** The QueryEngineConfigServiceImpl Logger. */
	private static final Logger LOGGER = LoggerFactory.getLogger(QueryEngineConfigServiceImpl.class);
	 ...
	 * 构造方法
	 */
	public QueryEngineConfigServiceImpl() {
	...
	public QUERY_CONFIG_CONNECTION[] getConfigConnectionByWhere(String strWhere) throws Exception {
		...
		try {
			if (strWhere.trim().length() == 0) {
				...
				strSql.append("Select * From QUERY_CONFIG_CONNECTION where " + strWhere);
			}
	...
	public boolean saveBaseProperty(QUERY_CONFIG_TABLE entityObj) throws Exception {
		...
		//dbServer.saveBaseProperty(entityObj);
		StringBuffer strSql = new StringBuffer();
		...
		if (mdbset != null) {
			...
			strSql.append(" Update QUERY_CONFIG_TABLE Set NAME='" + entityObj.getNAME() + "'" + ",MAINTABLE='"
					+ entityObj.getMAINTABLE()).append("'" + ",TYPE='" + entityObj.getTYPE() + "'")
					.append(",BID='" + entityObj.getBID() + "'").append(",CID='" + entityObj.getCID() + "'")
					.append(",IFFIRSTDATA='" + entityObj.getIFFIRSTDATA())
					.append("'" + ",IFMULSEL='" + entityObj.getIFMULSEL())
					.append("'" + ",IFCOMBTN ='" + entityObj.getIFCOMBTN())
					.append("'" + ",QITEMTYPE ='" + entityObj.getQITEMTYPE())
					.append("'" + ",QTABLETYPE ='" + entityObj.getQTABLETYPE())
					.append("'" + "  Where ID='" + entityObj.getID() + "'");
		} else {
			strSql.delete(0, strSql.length()); // 清空
			strSql.append(
					"Insert Into QUERY_CONFIG_TABLE(ID,NAME,MAINTABLE,TYPE,BID,CID,IFFIRSTDATA,IFMULSEL,IFCOMBTN,QITEMTYPE,QTABLETYPE) values('")
					.append(entityObj.getID() + "','" + entityObj.getNAME() + "','" + entityObj.getMAINTABLE() + "','"
							+ entityObj.getTYPE() + "','" + entityObj.getBID() + "','")
					.append(entityObj.getCID() + "','")
					.append(entityObj.getIFFIRSTDATA() + "'," + "'" + entityObj.getIFMULSEL() + "',");
			strSql.append("'" + entityObj.getIFCOMBTN() + "','" + entityObj.getQITEMTYPE() + "','"
					+ entityObj.getQTABLETYPE() + "')");
		}
		mdbset = null;
		try {
			result = dbEngine.ExecuteSQL(strSql.toString());
		} catch (Exception e) {
			result = false;
		}
		return result;
	...
	public QUERY_CONFIG_INIT[] getConfigInitByWhere(String strWhere) throws Exception {
		...
		try {
			...
			LOGGER.error("getConfigInitByWhere出现异常！" + e.getMessage());
			return null;
		}
		return entityList;
	}
...
//		int i;
		...
		try {
			...
			for (int i = 0; i < tempArray.length; i++) {
				...
				strSql = "Insert Into QUERY_CONFIG_QUERYFIELD(ID,FID,FIELD,ISPRECISION,ISMUST,ISDAY,DVALUE) Values('"
						+ getMaxFieldNoAdd(MaxFieldNo, 8, i) + "','" + FID + "','" + tempArray[i] + "','" 
						+ ISPRECISION + "','" + ISMUST + "','" + ISDAY + "','" + DVALUE + "')";
	...
	public QUERY_CONFIG_SHOWFIELD[] getResultFieldByWhere(String strWhere) throws Exception {
		...
		try {
			if (strWhere.trim().length() == 0) {
				...
				strSql.append("Select * From QUERY_CONFIG_SHOWFIELD where " + strWhere);
			}
			strSql.append(" Order by ID");
			List<Map<String, Object>> retlist = userMapper.selectListMapExecSQL(strSql.toString());
			int length = retlist != null ? retlist.size() : 0;
	...
	@Override
	public boolean saveDetailParameterProperty(String FID, 
			String FieldInfoStr, String BRelationInfoStr) throws Exception {
		// 参数名#参数字段名#按钮ID&
		// 按钮ID,按钮ID
		boolean result = false;
		String tempArray1[] = FieldInfoStr.split("&");
	...
	public boolean deleteConfigByID(String FID) throws Exception {
		...
		try {
			strSql.append("Delete From QUERY_CONFIG_PARAMETER Where FID='" + FID + "';"
						+ "Delete From QUERY_CONFIG_CONNECTION Where FID='" + FID + "';" 
						+ "Delete From QUERY_CONFIG_INIT Where FID='" + FID + "';" 
						+ "Delete From QUERY_CONFIG_QUERYFIELD Where FID='" + FID + "';" 
						+ "Delete From QUERY_CONFIG_SHOWFIELD Where FID='" + FID + "';" 
						+ "Delete From QUERY_CONFIG_BRELATION Where FID='" + FID + "';" 
						+ "Delete From QUERY_CONFIG_ORDER Where FID='" + FID + "';"
						+ "Delete From QUERY_CONFIG_BUTTON Where FID='" + FID + "';"
						+ "Delete From QUERY_CONFIG_TABLE Where ID='" + FID + "';");
			
			String[] sqlArray = strSql.toString().split(";");
	...
	public List<Map<String, Object>> getFieldList(String TableName) throws Exception {
		List<Map<String, Object>> result = new ArrayList<Map<String, Object>>();
		StringBuffer strSql = new StringBuffer();
		strSql.append("select FieldID,FieldName,CHINESENAME from bpip_field Where TABLEID=(Select TableID From Bpip_Table "
					+ "Where TableName='" + TableName.toUpperCase() + "') And FIELDTYPE!='3' Order by FieldID");
		List<Map<String, Object>> retlist = userMapper.selectListMapExecSQL(strSql.toString());
		int length = retlist != null ? retlist.size() : 0;
		if (length > 0) {
			for (int i = 0; i < length; i++) {
	...
	@Override
	public List<Map<String, Object>> getTableFieldList(String TableName, boolean flag) throws Exception {
		List<Map<String, Object>> result = new ArrayList<Map<String, Object>>();
		String strSql = "";
		String strSqlA = "";
		String strTMP = "";
		...
		if (tableArray.length > 0) {
			...
			if (DataBaseType.equals("3")) {// mysql数据库
				...
				strSqlA = "Select b.Tablename,a.FieldName,a.fieldtype, a.CHINESENAME  FROM bpip_field a,bpip_table b where a.Tableid=b.tableid And b.TableName='"
						+ tmpTableName + "' And a.FIELDTYPE!='3' Order By b.TableId,a.FieldId";
				dbset1 = dbEngine.QuerySQL(strSqlA);
			} else {
				strSql = "Select b.Tablename,a.FieldName,a.fieldtype, b.CHINESENAME as tableCHINESENAME,a.CHINESENAME as fieldCHINESENAME "
						+ "FROM bpip_field a,bpip_table b where a.Tableid=b.tableid And b.TableName='" + tmpTableName + "' And a.FIELDTYPE!='3' Order By b.TableId,a.FieldId";
			}
			dbset = dbEngine.QuerySQL(strSql);
			if (dbset != null && dbset.RowCount() > 0) {
				for (int i = 0; i < dbset.RowCount(); i++) {
					String[] saTmp = new String[2];
					strTMP = dbset.Row(i).Column("Tablename").getString() + "." + dbset.Row(i).Column("FieldName").getString();
					saTmp[0] = strTMP;
					if (flag) {
						strTMP = saTmp[0] + "." + dbset.Row(i).Column("fieldtype").getString();
						saTmp[0] = strTMP;
					}
					if (DataBaseType.equals("3")) {// mysql数据库
						strTMP = dbset.Row(i).Column("CHINESENAME").getString() + "." + dbset1.Row(i).Column("CHINESENAME").getString();
					} else {
						strTMP = dbset.Row(i).Column("tableCHINESENAME").getString() + "." + dbset.Row(i).Column("fieldCHINESENAME").getString();
					}
					saTmp[1] = strTMP;
					result.add(new HashMap<String, Object>(){
						/**
			...
			if (tableArray.length > 1) {
				...
				if (DataBaseType.equals("3")) {// mysql数据库
					...
					strSql = "Select b.Tablename,a.FieldName,a.fieldtype, b.CHINESENAME as tableCHINESENAME,a.CHINESENAME as fieldCHINESENAME FROM bpip_field a,bpip_table b "
							+ "where a.Tableid=b.tableid And b.TableName In(" + tmpTableName + ") And a.FIELDTYPE!='3' Order By b.TableId,a.FieldId";
				}
				dbset = dbEngine.QuerySQL(strSql);
				if (dbset != null && dbset.RowCount() > 0) {
					for (int i = 0; i < dbset.RowCount(); i++) {
	...
	public String getTableFieldCnNameByEn(String tableFieldName) throws Exception {
		...
		if (DataBaseType.equals("1")) {// oracle数据库
			...
			strSql = "select b.CHINESENAME FROM bpip_field a,bpip_table b where a.Tableid=b.tableid "
					+ "and concat(b.tablename,'.',a.fieldname)='" + tableFieldName + "'";
			strSqlA = "select a.CHINESENAME FROM bpip_field a,bpip_table b where a.Tableid=b.tableid "
					+ "and concat(b.tablename,'.',a.fieldname)='" + tableFieldName + "'";
	...
	public String max_OrderNo(String prentNo) throws Exception {
		...
		try {
			...
			if (dbset != null) {
				strMaxNO = dbset.Row(0).Column("MAX_NO").getString();
			...
			LOGGER.error("max_OrderNo出现异常！" + ex.getMessage());
		}
		return strMenu_No;
	}
	...
	public boolean querycopy(String strID) throws Exception {
		...
		if (retmap != null && retmap.size() > 0) {
			...
			strFID = strMaxNo;// 获取复制记录的ID
			retmap.put("ID", strMaxNo);// 设置ID
			strTmp = retmap.get("NAME").toString();
			retmap.put("NAME", "复件_" + strTmp);// 设置名称
			ReflectionUtil.handleParaMap(retmap, QUERY_CONFIG_TABLE.class);
		...
		if (strFID.length() > 0) {
			...
			if (length1 > 0) {
				for (int i = 0; i < length1; i++) {
					...
					queryConfigMapper.insertQUERY_CONFIG_SHOWFIELD(retmap1);
				}
				retlist = null;
			}
			// ----------------------------------------
			...
			retlist = userMapper.selectListMapExecSQL(strSql);
			int length2 = retlist != null ? retlist.size() : 0;
			if (length2 > 0) {
				for (int i = 0; i < length2; i++) {
					Map<String, Object> retmap2 = retlist.get(i);
			...
			retlist = userMapper.selectListMapExecSQL(strSql);
			int length5 = retlist != null ? retlist.size() : 0;
			if (length5 > 0) {
				for (int i = 0; i < length5; i++) {
					Map<String, Object> retmap5 = retlist.get(i);
			...
			retlist = userMapper.selectListMapExecSQL(strSql);
			int length6 = retlist != null ? retlist.size() : 0;
			if (length6 > 0) {
				for (int i = 0; i < length6; i++) {
					Map<String, Object> retmap6 = retlist.get(i);
			...
			retlist = userMapper.selectListMapExecSQL(strSql);
			int length7 = retlist != null ? retlist.size() : 0;
			if (length7 > 0) {
				for (int i = 0; i < length7; i++) {
					Map<String, Object> retmap7 = retlist.get(i);
			...
			retlist = userMapper.selectListMapExecSQL(strSql);
			int length8 = retlist != null ? retlist.size() : 0;
			if (length8 > 0) {
				for (int i = 0; i < length8; i++) {
					Map<String, Object> retmap8 = retlist.get(i);
